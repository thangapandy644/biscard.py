...
if choice == 'Add':
    ...
    if st.button("Update Business Card"):
     try:
        ...
        sql_query = "UPDATE card_data SET name=%s, job_title=%s, address=%s, postcode=%s, phone=%s, email=%s, website=%s, company_name=%s WHERE name=%s"
        data = (name, job_title, address, postcode, phone, email, website, company_name, selected_card_name)
